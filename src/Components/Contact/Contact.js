...
const Contact = () => {
  window.onload=function(){
    const buttonds = document.querySelector(".buttonds");
    const toast = document.querySelector(".toast");
    const closeIcon = document.querySelector(".clos");
    const progre = document.querySelector(".progre");
 
     buttonds.addEventListener("click", () => {
       toast.classList.add("active");
       progre.classList.add("active");

       setTimeout(() => {
          toast.classList.remove("active");
       }, 5000);
       setTimeout(() => {
        progre.classList.remove("active");
      }, 5300);
     });
  ...
  return (
        ...
        <div className="formm" ref={ref}>
          <motion.div animate={ani} className="left">
          <div className="opo">
          ...
          <motion.div animate={animation} className="right">
          {/* <div className="toast">
              <div className="toast-content">
                <div className="che"><MdCheck /></div>
                <div className="messa">
                  <span className="text text-1">Thank You!</span>
                  <span className="text text-2">Your Message Has Been Sent</span>
                </div>
              </div>
              <div className="clos"><MdClose /></div>
